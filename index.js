...
// @version     1.0
// @author      Ltwoz
// @homepageURL https://github.com/Ltwoz/notcoin-bot
// ==/UserScript==
...
const getEvent = () => ({
    touches: [
        {
            clientX: Math.random() * 250 + 52,
            clientY: Math.random() * 582 + 512,
        },
    ],
});
const getReactProps = (component) =>
    component
        ? Object.entries(component).find(([key]) =>
              key.startsWith("__reactProps$")
          )[1]
        : null;
const getIdleTime = () => 5 * 60 * 1000;
...
const MIN_SCORE = 100;
const MAX_CLICKS_PER_ITERATION = 1000;
...
const boostClick = async () => {
    ...
    boostButton.onClick();
    await delay(BOOST_CLICK_TIMEOUT);
    const dailyTask = document.querySelector(
        'div[class^="_taskDailyItem"]:not([class*="_completed"])'
    );
    const dailyBoost = getReactProps(dailyTask);
    const taskCarousel = document.querySelector(
        'div[class^="_taskCarousel"][class*="_willChange"]'
    );
    const boost = getReactProps(
        taskCarousel &&
            taskCarousel.querySelector('div[class*="_rippleEffect"]')
    );
    const boosterButton = dailyBoost || boost;
    if (boosterButton) {
        boosterButton.onClick();
        await delay(BOOST_CLICK_TIMEOUT);
        const getButton = getReactProps(
            document.querySelector('button[class*="_typeBlue"]')
        );
...
if (location.host === "clicker.joincommunity.xyz") {
    waitForElm('div[class^="_notcoin"]', 10000)
        .then(start)
        .catch(() => {
            console.info(
                "%c INFO: not started. reload window",
                "color: #64b5f6"
            );
            location.reload();
        });
